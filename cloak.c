...
typedef struct {
    ...
    int  calibrate;
    int  fragment_real;
    int  fragment_lengths[MAX_FRAG_LENGTHS]; /* byte sizes of each chunk, in order */
    int  fragment_length_count;              /* 0 = use the old built-in default */

    uint16_t decoy_ciphers[MAX_CIPHERS]; /* cipher suite list for the DECOY hello only --
                                             the real hello's ciphers come from your VPN
                                             client (v2rayNG/Xray) and cloak can't change them */
...
/* Fragment chunk sizes, e.g. "5,94,1" -- first chunk is 5 bytes, next
 * is 94, next is 1, and any leftover bytes beyond that go out as one
...
static void parse_fragment_lengths(const char *value) {
    ...
    while (tok && g_cfg.fragment_length_count < MAX_FRAG_LENGTHS) {
        int v = atoi(trim(tok));
        if (v > 0) {
            g_cfg.fragment_lengths[g_cfg.fragment_length_count] = v;
...
/* Cipher suite list for the DECOY ClientHello, as 4-hex-digit codes,
 * e.g. "1301,1302,c02b". This only changes the fingerprint of
 * cloak's own decoy handshake -- it cannot change the real hello's
 * ciphers, since that hello is built by your VPN client, not cloak. */
...
static size_t build_fake_client_hello(uint8_t *out, size_t cap, const char *sni) {
     ...
     * If the user set decoy_ciphers in the config, use that list
     * instead -- this only changes the DECOY's fingerprint, not the
     * real hello's (that one comes from your VPN client as-is). */
...
static void send_fragmented(int fd, const uint8_t *data, size_t len) {
    ...
    if (g_cfg.fragment_length_count > 0) {
        /* Use the user-configured split points, e.g. from fragment_lengths
         * = 5,94,1 -- same idea as PattNG's finalMask "lengths" field. */
        ...
        /* Anything left over after the configured list goes out as one
         * final chunk, so a short list doesn't silently drop data. */
        if (off < len) send_all(fd, data + off, len - off);
        return;
    }

    /* No fragment_lengths configured -- fall back to the old built-in
     * default: a tiny first chunk (splits the SNI), then bigger ones. */
    size_t chunk = 4;
    while (off < len) {
        size_t n = (len - off < chunk) ? (len - off) : chunk;
        send_all(fd, data + off, n);
